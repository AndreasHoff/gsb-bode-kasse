---
status: implemented
implemented_date: 2026-08-27
implemented_by: Copilot
note: "PWA installation prompt feature with platform-specific handling for Android and iOS"
---
...
### Created Files
- `src/features/pwa-install/usePWAInstall.ts` - Hook for PWA installation detection and platform-specific handling
- `src/features/pwa-install/InstallPrompt.tsx` - Installation banner for WelcomeAuth screen
- `src/features/pwa-install/InstallAppOption.tsx` - Installation option for Profile screen
- `src/features/pwa-install/pwa-install.css` - Feature-specific styling
...
- Used `beforeinstallprompt` event for Android native installation
- Used `window.matchMedia("(display-mode: standalone)")` and `navigator.standalone` for PWA detection
- Stored dismissal preference in `localStorage` with key `gsb:pwa-install-dismissed`
- Platform detection based on user agent string
- No ActivityLog entries (not a data mutation feature)

### No Data Model Changes
This feature does not interact with Firestore or modify any domain entities.
