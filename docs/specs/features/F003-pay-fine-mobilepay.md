...
## Problem
Players need a frictionless way to pay outstanding fines. The app must not act as a payment processor but should route users to MobilePay with prefilled details.

## Goal
Allow a player to initiate payment for one or more outstanding fines via MobilePay deep-link, then have an admin confirm receipt.

## Actors
- Player (initiates)
- Admin / Treasurer (approves)

## Preconditions
- Player has at least one unpaid fine
- Team has a configured MobilePay recipient number

## Flow

### Player Side
1. Player opens personal debt overview
2. Views list of unpaid fines
3. Taps "Pay" on one or more fines (or "Pay All")
4. App constructs MobilePay deep-link with:
   - Total amount
   - Recipient number (team treasurer)
...
7. Player returns to app after completing payment
...
13. Player's debt overview reflects cleared fine
